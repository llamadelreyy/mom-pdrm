...
// Constants - exact from HTML version
...
const PDRMApp = () => {
  ...
  const generateMarkdownPreview = () => {
    if (!processedTranscription) return 'Pratonton Markdown akan dipaparkan di sini…';
    ...
    return `
      <div class="header">
        ...
        ${metadata.rujukan ? `<div class="ref">Rujukan: ${metadata.rujukan}</div>` : ''}
      </div>
      <table class="meta">
        <tr><td>Bil. Mesyuarat</td><td>: ${metadata.bil}</td></tr>
        <tr><td>Tarikh</td><td>: ${metadata.tarikh}</td></tr>
        <tr><td>Masa</td><td>: ${metadata.masa}</td></tr>
        <tr><td>Tempat</td><td>: ${metadata.tempat}</td></tr>
      </table>
      <div>${processedTranscription.replace(/\n/g, '<br/>')}</div>
    `;
  };
