...
const StatusToast = ({ status, message, isVisible }) => {
...
const PDRMApp = () => {
  ...
  const downloadProcessedTranscription = async (format) => {
    ...
    try {
      ...
      fd.append('template', 'pdrm');
      fd.append('title', extractTitleFromContent(processedTranscription) || 'MINIT MESYUARAT');
  ...
  // Extract title from content (same logic as HTML version)
  ...
  return (
    <div>
      {/* Status Toast */}
      <StatusToast status={status.type} message={status.message} isVisible={status.visible} />
              ...
              <div>
                ...
                <form onSubmit={handleTranscription}>
                  <div className="form-row">
                    <UploadBox onFileSelect={setSelectedFile} selectedFile={selectedFile} />
                    <div>
                      <label htmlFor="maxWorkers">Prestasi (Bil. Pekerja)</label>
                      <input 
                        className="control" type="number" min="1" max="16" 
                ...
                <div className="card" style={{marginTop: '32px'}}>
                  <div className="card-hd"><h2>Keputusan Transkripsi</h2></div>
                  <div className="card-bd">
                    ...
                    <div className="btn-group" style={{marginTop: '16px'}}>
                      ...
                      <button className="btn btn-ghost" onClick={() => downloadTranscription('docx')}>Muat turun DOCX</button>
                      <button className="btn btn-ghost" onClick={() => downloadTranscription('pdf')}>Muat turun PDF</button>
                      <button className="btn btn-ghost" onClick={() => downloadTranscription('txt')}>Muat turun TXT</button>
                    </div>
                  </div>
                </div>
              </div>
                  ...
                  <div className="card-bd">
                    <div className="form-row">
                      <div>
                        ...
                        <input type="text" className="control" placeholder="Contoh: PDRM/MM/2025/001"
                          value={metadata.rujukan} onChange={(e) => setMetadata({...metadata, rujukan: e.target.value})} />
                      </div>
                      <div>
                        <label htmlFor="bil">Bil. Mesyuarat</label>
                        <input type="text" className="control" placeholder="Contoh: 1/2025"
                          value={metadata.bil} onChange={(e) => setMetadata({...metadata, bil: e.target.value})} />
                      </div>
                    </div>
                    <div className="form-row">
                      <div>
                        <label htmlFor="tarikh">Tarikh</label>
                        <input type="text" className="control" placeholder="Contoh: 5 November 2025"
                          value={metadata.tarikh} onChange={(e) => setMetadata({...metadata, tarikh: e.target.value})} />
                      </div>
                      <div>
                        <label htmlFor="masa">Masa</label>
                        <input type="text" className="control" placeholder="Contoh: 10:00 Pagi - 12:00 Tengahari"
                          value={metadata.masa} onChange={(e) => setMetadata({...metadata, masa: e.target.value})} />
                      </div>
                    </div>
                    <div className="form-row">
                      <div>
                        <label htmlFor="tempat">Tempat</label>
                        <input type="text" className="control" placeholder="Contoh: Bilik Mesyuarat Utama, IPK"
                          value={metadata.tempat} onChange={(e) => setMetadata({...metadata, tempat: e.target.value})} />
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div className="card">
                  <div className="card-hd">
                    <h2>
                      <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                        <path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/>
                        <path d="M14 2v6h6"/><path d="M8 13h8M8 17h8M8 9h2"/>
                      </svg>
                  ...
                  <div className="card-bd">
                    <div style={{display:'flex', alignItems:'center', justifyContent:'space-between', gap: '12px', marginBottom: '12px'}}>
                      ...
                      <div className="btn-group">
                        <select className="control" style={{padding:'8px 10px', width: 'auto', minWidth: '240px'}}
                          value={selectedProm